...
func TestNew(t *testing.T) {
	...
	logger, err := logging.New(logging.Info, newWriter(), 0)
...
func TestLog_SetVerbosity(t *testing.T) {
	...
	logger.Debug("test")
	assert.Equal(t, []byte{}, writer.read())
	...
	assert.Contains(t, string(writer.read()), "[debug] test")
}
...
func TestLog_format(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"pkg public": {
			expected: "error",
			message:  errors.Public(errPackage, 1, "test"),
		},
		"stdlib error": {
			expected: "error",
			message:  errStdlib,
	...
	for name, test := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			assert.Contains(t, string(writer.read()), "[info] "+test.expected)
		})
	}
}
...
func testLogLevel(t *testing.T, testcase testcase) {
	...
	defer func() {
		...
		assert.Contains(t, string(writer.read()), testcase.expected.logLevel)
	}()
...
func testVerbosity(t *testing.T, testcase testcase) {
	...
	logger.Debug("test")
	assert.Contains(t, string(writer.read()), testcase.expected.verbosity[logging.Debug])

	logger.Info("test")
	assert.Contains(t, string(writer.read()), testcase.expected.verbosity[logging.Info])

	logger.Error("test")
	assert.Contains(t, string(writer.read()), testcase.expected.verbosity[logging.Error])

	logger.Warn("test")
	assert.Contains(t, string(writer.read()), testcase.expected.verbosity[logging.Warn])
}
